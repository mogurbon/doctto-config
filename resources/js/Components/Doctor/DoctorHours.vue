		...
		<div class="border-b border-gray-200 mb-4">
			<nav class="-mb-px flex space-x-4 overflow-x-auto">
				<button 
                    ...
                    :class="activeDay === day.key ? 'border-cyan-500 text-cyan-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                >
                    {{ day.label }}
                </button>
			</nav>
		...
		<div v-if="activeDay" class="bg-slate-50 p-4 rounded-lg border border-gray-200 mb-6">
			<div class="flex items-center justify-between mb-4">
				<label class="relative inline-flex items-center cursor-pointer">
					<input type="checkbox" :checked="Array.isArray(model[activeDay]) && model[activeDay].length > 0" @change="$event.target.checked ? (model[activeDay] = [{ start: '09:00', end: '18:00' }]) : (model[activeDay] = [])" class="sr-only peer">
					...
					<span class="ml-3 text-sm font-semibold text-gray-900">¿Abre este día? ({{ days.find(d => d.key === activeDay).label }})</span>
				</label>
			</div>
			...
			<div v-if="Array.isArray(model[activeDay])">
				<div v-for="(block, idx) in model[activeDay]" :key="idx" class="flex items-center gap-3 mb-3">
					...
					<div class="flex items-center">
						...
						<input type="time" v-model="block.start" class="border-gray-200 rounded-md shadow-sm focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm">
					</div>
					<div class="flex items-center">
						<span class="text-sm text-gray-500 mr-2">Fin</span>
						<input type="time" v-model="block.end" class="border-gray-200 rounded-md shadow-sm focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm">
					</div>
			...
			<div v-if="Array.isArray(model[activeDay])" class="mt-4">
